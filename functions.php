...
function business_scripts_styles() {
	...
	// Enqueue Google fonts.
	wp_enqueue_style( 'google-fonts', '//fonts.googleapis.com/css?family=Roboto:300,400,500,700|Hind:400', array(), CHILD_THEME_VERSION );
...
function custom_load_custom_style_sheet() {
  wp_enqueue_style( 'custom-stylesheet', CHILD_URL . '/assets/css/custom.css', array(), CHILD_THEME_VERSION );
}

//* Load custom style sheet on other pages

add_action( 'wp_enqueue_scripts', 'load_frontpage_home_custom_stylesheet' );
function load_frontpage_home_custom_stylesheet() {
    if ( !is_front_page() ) {
    wp_enqueue_style( 'other-style', get_stylesheet_directory_uri() . '/assets/css/other.css', array(), '1.0.0' );
    }
}
...
add_action( 'genesis_header', 'business_reposition_page_title', 9 );

function business_reposition_page_title() {

	// Reposition post titles back inside posts loop.
	if ( ! is_singular('portfolio') ) {
					return;
	}
...
function themeprefix_genesis_post_info_filter( $post_info ) {
	if ( 'portfolio' == get_post_type() ) { //swap in CPT name
		$post_info='<span class="author"></span> <span class="categories">[post_terms taxonomy="portfolio-type" before=""]</span> <span class="date">[post_modified_date]</span>';//swap in taxonomy and label name
		return $post_info;
	}
...
function sp_footer_creds_filter( $creds ) {
	$creds = '[footer_copyright before="Copyright "] · Grace Marketing Agency';
	return $creds;
}
